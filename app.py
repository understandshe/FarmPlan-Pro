...
def draw_layout_map(layout_data: dict) -> bytes:
    ...
    fig.text(0.5, 0.97, f"{acres} Acre {farm_type} Layout Plan — {country}", ha="center", va="top", fontsize=15, fontweight="bold", color="#1A252F")
    fig.text(0.5, 0.935, f"Total Area: {layout_data['total_sqm']:,.0f} m² | {layout_data['total_sqft']:,.0f} sq ft | Budget: {layout_data['budget']}", ha="center", va="top", fontsize=9, color="#555555")
    ...
    def draw_column(zone_list, x_start, col_width):
        ...
        for z in zone_list:
            ...
            rect = mpatches.FancyBboxPatch((x_start, y_cursor - h), col_width, h, boxstyle="round,pad=0.3", linewidth=1.2, edgecolor=SECTION_EDGE_COLOR, facecolor=color, zorder=2)
            ax.add_patch(rect)
            ...
            ax.text(x_start + col_width / 2, y_cursor - h / 2 + 1.5, label, ha="center", va="center", fontsize=font_size, fontweight="bold", color="#1A252F", zorder=3, wrap=True, multialignment="center")
            ax.text(x_start + col_width / 2, y_cursor - h / 2 - 1.8, f"{z['sqm']:,.0f} m²", ha="center", va="center", fontsize=6.5, color="#555555", style="italic", zorder=3)
    ...
    ax.add_patch(mpatches.FancyBboxPatch((LEFT_X + LEFT_W, MARGIN_BOT), RIGHT_X - (LEFT_X + LEFT_W), MARGIN_TOP - MARGIN_BOT, boxstyle="square,pad=0", linewidth=0, facecolor="#D5D8DC", zorder=1))
    ax.text((LEFT_X + LEFT_W + RIGHT_X) / 2, (MARGIN_TOP + MARGIN_BOT) / 2, "SERVICE\nLANE", ha="center", va="center", fontsize=6, color="#7F8C8D", fontweight="bold", rotation=90, zorder=4)
    
    arrow_props = dict(arrowstyle="->", color="#2E86C1", lw=1.5, connectionstyle="arc3,rad=0.0")
    for ay_pos in [75, 55, 35]:
        ax.annotate("", xy=(LEFT_X + LEFT_W - 0.5, ay_pos - 6), xytext=(LEFT_X + LEFT_W - 0.5, ay_pos), arrowprops=arrow_props, zorder=5)
    
    cx, cy, r = 96, 95, 3
    ax.annotate("N", xy=(cx, cy + r), xytext=(cx, cy), ha="center", va="center", fontsize=8, fontweight="bold", arrowprops=dict(arrowstyle="->", color="#C0392B", lw=2), color="#C0392B", zorder=6)
    
    border = mpatches.FancyBboxPatch((0.3, 0.3), 99.4, 99.4, boxstyle="round,pad=0.1", linewidth=2.5, edgecolor="#1A252F", facecolor="none", zorder=10)
    ax.add_patch(border)
    
    legend_patches = [mpatches.Patch(facecolor=ZONE_COLORS.get(z["name"], "#D5D8DC"), edgecolor=SECTION_EDGE_COLOR, linewidth=0.8, label=z["name"]) for z in zones]
    ax.legend(handles=legend_patches, loc="lower center", bbox_to_anchor=(0.5, -0.11), ncol=5, fontsize=6.5, framealpha=0.9, edgecolor="#BDC3C7", title="Zone Legend", title_fontsize=7)
...
def _build_styles():
    ...
    return {
        "doc_title": ParagraphStyle("DocTitle", parent=base["Title"], fontSize=22, textColor=C_WHITE, alignment=TA_CENTER, spaceAfter=4, spaceBefore=4, fontName="Helvetica-Bold"),
        "doc_subtitle": ParagraphStyle("DocSubtitle", parent=base["Normal"], fontSize=10, textColor=colors.HexColor("#BDC3C7"), alignment=TA_CENTER, spaceAfter=2, fontName="Helvetica"),
        "section_heading": ParagraphStyle("SectionHeading", parent=base["Heading2"], fontSize=13, textColor=C_BLUE, spaceBefore=14, spaceAfter=6, fontName="Helvetica-Bold"),
        "body": ParagraphStyle("Body", parent=base["Normal"], fontSize=9, textColor=C_DARK, leading=14, fontName="Helvetica"),
        "body_bold": ParagraphStyle("BodyBold", parent=base["Normal"], fontSize=9, textColor=C_DARK, fontName="Helvetica-Bold"),
        ...
        "footer": ParagraphStyle("Footer", parent=base["Normal"], fontSize=7, textColor=C_GREY, alignment=TA_CENTER, fontName="Helvetica"),
    }
...
def _header_block(styles, layout_data):
    ...
    header_table = Table([[Paragraph(title_text, styles["doc_title"])], [Paragraph(subtitle_text, styles["doc_subtitle"])]], colWidths=[170 * mm])
    header_table.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, -1), C_DARK), ("TOPPADDING", (0, 0), (-1, -1), 14), ("BOTTOMPADDING", (0, -1), (-1, -1), 14), ("LEFTPADDING", (0, 0), (-1, -1), 16), ("RIGHTPADDING", (0, 0), (-1, -1), 16)]))
...
def _summary_bar(styles, layout_data):
    story = []
    kpis = [("Total Land Area", f"{layout_data['land_acres']} acres"), ("Total Area (m²)", f"{layout_data['total_sqm']:,.0f} m²"), ("Total Area (sqft)", f"{layout_data['total_sqft']:,.0f} sq ft"), ("Farm Type", layout_data["farm_type"]), ("Region/Country", layout_data["country"]), ("Budget Level", layout_data["budget"])]
    data = [[Table([[Paragraph(v, ParagraphStyle("kv", fontSize=11, fontName="Helvetica-Bold", textColor=C_BLUE))], [Paragraph(k, ParagraphStyle("kk", fontSize=7.5, fontName="Helvetica", textColor=C_GREY))]], colWidths=[52 * mm]) for k, v in kpis[:3]], [Table([[Paragraph(v, ParagraphStyle("kv2", fontSize=10, fontName="Helvetica-Bold", textColor=C_GREEN))], [Paragraph(k, ParagraphStyle("kk2", fontSize=7.5, fontName="Helvetica", textColor=C_GREY))]], colWidths=[52 * mm]) for k, v in kpis[3:]]]
    kpi_table = Table(data, colWidths=[56 * mm, 56 * mm, 56 * mm])
    kpi_table.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, -1), C_LIGHT), ("BOX", (0, 0), (-1, -1), 0.8, C_BLUE), ("INNERGRID", (0, 0), (-1, -1), 0.5, colors.HexColor("#BDC3C7")), ("TOPPADDING", (0, 0), (-1, -1), 8), ("BOTTOMPADDING", (0, 0), (-1, -1), 8), ("LEFTPADDING", (0, 0), (-1, -1), 10), ("ALIGN", (0, 0), (-1, -1), "CENTER"), ("VALIGN", (0, 0), (-1, -1), "MIDDLE")]))
...
def _area_table(styles, layout_data):
    ...
    header_row = [Paragraph("<b>Zone / Section</b>", styles["body_bold"]), Paragraph("<b>% Share</b>", styles["body_bold"]), Paragraph("<b>Area (m²)</b>", styles["body_bold"]), Paragraph("<b>Area (sq ft)</b>", styles["body_bold"]), Paragraph("<b>Animal Capacity</b>", styles["body_bold"])]
    rows = [header_row]
    for i, zone in enumerate(layout_data["zones"]):
        ...
        rows.append([Paragraph(zone["name"], styles["body"]), Paragraph(f"{zone['fraction']*100:.1f}%", styles["body"]), Paragraph(f"{zone['sqm']:,.1f}", styles["body"]), Paragraph(f"{zone['sqft']:,.0f}", styles["body"]), Paragraph(cap_str, styles["body"])])
    col_widths = [65 * mm, 22 * mm, 28 * mm, 28 * mm, 32 * mm]
    tbl = Table(rows, colWidths=col_widths, repeatRows=1)
    tbl.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, 0), C_DARK), ("TEXTCOLOR", (0, 0), (-1, 0), C_WHITE), ("ROWBACKGROUNDS", (0, 1), (-1, -1), [C_ROW_A, C_ROW_B]), ("BOX", (0, 0), (-1, -1), 0.8, C_DARK), ("INNERGRID", (0, 0), (-1, -1), 0.4, colors.HexColor("#BDC3C7")), ("TOPPADDING", (0, 0), (-1, -1), 5), ("BOTTOMPADDING", (0, 0), (-1, -1), 5), ("LEFTPADDING", (0, 0), (-1, -1), 6), ("VALIGN", (0, 0), (-1, -1), "MIDDLE"), ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"), ("FONTSIZE", (0, 0), (-1, 0), 9), ("ALIGN", (1, 0), (-1, -1), "CENTER")]))
    story.append(tbl)
    story.append(Spacer(1, 6 * mm))
    return story
...
def _infra_table(styles, layout_data):
    ...
    infra_rows = [[Paragraph("<b>Item</b>", styles["body_bold"]), Paragraph("<b>Status</b>", styles["body_bold"]), Paragraph("<b>Specification / Note</b>", styles["body_bold"])], [Paragraph("Construction Quality", styles["body"]), Paragraph("Defined", styles["body"]), Paragraph(bi["quality"], styles["body"])], [Paragraph("Flooring Type", styles["body"]), Paragraph("Defined", styles["body"]), Paragraph(bi["flooring"], styles["body"])], [Paragraph("Roofing Type", styles["body"]), Paragraph("Defined", styles["body"]), Paragraph(bi["roofing"], styles["body"])]]
    for item_name, status, note in layout_data["infra_details"]:
        infra_rows.append([Paragraph(item_name, styles["body"]), Paragraph(f'<font color="#1E8449"><b>{status}</b></font>', styles["body"]), Paragraph(note, styles["body"])])
    col_widths = [55 * mm, 28 * mm, 92 * mm]
    tbl = Table(infra_rows, colWidths=col_widths, repeatRows=1)
    tbl.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, 0), C_GREEN), ("TEXTCOLOR", (0, 0), (-1, 0), C_WHITE), ("ROWBACKGROUNDS", (0, 1), (-1, -1), [C_ROW_A, C_ROW_B]), ("BOX", (0, 0), (-1, -1), 0.8, C_DARK), ("INNERGRID", (0, 0), (-1, -1), 0.4, colors.HexColor("#BDC3C7")), ("TOPPADDING", (0, 0), (-1, -1), 5), ("BOTTOMPADDING", (0, 0), (-1, -1), 5), ("LEFTPADDING", (0, 0), (-1, -1), 6), ("VALIGN", (0, 0), (-1, -1), "MIDDLE")]))
    story.append(tbl)
    story.append(Spacer(1, 6 * mm))
    return story
...
def _animal_summary(styles, layout_data):
    ...
    labels = {"milch_cows": "Milch Cows (Lactating)", "dry_cows": "Dry Cows", "calves": "Calves", "goats": "Goats", "chickens": "Chickens / Poultry"}
    animal_rows = [[Paragraph("<b>Animal Type</b>", styles["body_bold"]), Paragraph("<b>Count</b>", styles["body_bold"]), Paragraph("<b>Space Required (m²)</b>", styles["body_bold"])]]
    ...
    for key, label in labels.items():
        ...
        if count > 0:
            ...
            animal_rows.append([Paragraph(label, styles["body"]), Paragraph(str(count), styles["body"]), Paragraph(f"{count * sp:,.1f} m²", styles["body"])])
    col_widths = [80 * mm, 30 * mm, 65 * mm]
    tbl = Table(animal_rows, colWidths=col_widths, repeatRows=1)
    tbl.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, 0), C_ACCENT), ("TEXTCOLOR", (0, 0), (-1, 0), C_WHITE), ("ROWBACKGROUNDS", (0, 1), (-1, -1), [C_ROW_A, C_ROW_B]), ("BOX", (0, 0), (-1, -1), 0.8, C_DARK), ("INNERGRID", (0, 0), (-1, -1), 0.4, colors.HexColor("#BDC3C7")), ("TOPPADDING", (0, 0), (-1, -1), 5), ("BOTTOMPADDING", (0, 0), (-1, -1), 5), ("LEFTPADDING", (0, 0), (-1, -1), 6), ("ALIGN", (1, 0), (2, -1), "CENTER"), ("VALIGN", (0, 0), (-1, -1), "MIDDLE")]))
    story.append(tbl)
    story.append(Spacer(1, 6 * mm))
    return story
...
def _tips_section(styles, layout_data):
    ...
    for i, tip in enumerate(layout_data["tips"], 1):
        tip_row = Table([[Paragraph(f'<font color="#E67E22"><b>{i:02d}</b></font>', ParagraphStyle("tipnum", fontSize=10, fontName="Helvetica-Bold", alignment=TA_CENTER)), Paragraph(tip, styles["tip"])]], colWidths=[12 * mm, 155 * mm])
        tip_row.setStyle(TableStyle([("BACKGROUND", (0, 0), (-1, -1), colors.HexColor("#EBF5FB")), ("BOX", (0, 0), (-1, -1), 0.5, colors.HexColor("#AED6F1")), ("LEFTPADDING", (0, 0), (-1, -1), 6), ("RIGHTPADDING", (0, 0), (-1, -1), 6), ("TOPPADDING", (0, 0), (-1, -1), 5), ("BOTTOMPADDING", (0, 0), (-1, -1), 5), ("VALIGN", (0, 0), (-1, -1), "MIDDLE")]))
...
def _disclaimer_footer(styles):
    text = "This farm layout plan is generated based on standard agricultural zoning guidelines and the inputs provided. Actual construction should be verified by a qualified agricultural engineer or veterinary consultant. All area figures are approximate. Space requirements may vary based on local regulations and breed specifications."
    story = [HRFlowable(width="100%", thickness=0.5, color=C_GREY, spaceBefore=8, spaceAfter=4), Paragraph(text, styles["footer"])]
    return story

def generate_pdf(layout_data: dict, map_png_bytes: bytes) -> bytes:
    buf = io.BytesIO()
    styles = _build_styles()
    doc = SimpleDocTemplate(buf, pagesize=A4, leftMargin=18 * mm, rightMargin=18 * mm, topMargin=16 * mm, bottomMargin=16 * mm, title=f"{layout_data['land_acres']} Acre {layout_data['farm_type']} Plan", author="FarmPlan Pro", subject="Premium Farm Layout Plan")
    story = []
    ...
    story.append(rl_img)
    story.append(Spacer(1, 4 * mm))
    story.append(Paragraph(f"<i>Top-view 2D layout for a {layout_data['land_acres']}-acre {layout_data['farm_type']} in {layout_data['country']}. All zones are proportionally sized to actual land allocation.</i>", ParagraphStyle("caption", fontSize=7.5, textColor=C_GREY, alignment=TA_CENTER, fontName="Helvetica-Oblique")))
...
# =============================================================================
def main():
    st.set_page_config(page_title="FarmPlan Pro – Premium Farm Layout Generator", page_icon="🌾", layout="wide", initial_sidebar_state="expanded")
    st.markdown("""
        <style>
    ...
    if generate_btn:
        with st.spinner("Calculating layout and generating PDF…"):
            animals = {"milch_cows": milch_cows, "dry_cows": dry_cows, "calves": calves, "goats": goats, "chickens": chickens}
            infra_toggles = {"milking_area": milking_area, "isolation_pen": isolation_pen, "fodder_storage": fodder_storage, "water_system": water_system, "storage_barn": storage_barn}
        ...
        kpis = [(c1, f"{layout_data['land_acres']:.2f} acres", "Total Land"), (c2, f"{layout_data['total_sqm']:,.0f} m²", "Total Area (m²)"), (c3, f"{layout_data['total_sqft']:,.0f} sqft", "Total Area (sq ft)"), (c4, layout_data["budget"], "Budget Level")]
        ...
        with col_map:
            ...
            st.image(map_png, use_container_width=True, caption=f"{land_size} {land_unit} {farm_type} – {country}")
        with col_info:
            st.markdown("### 📊 Zone Breakdown")
            for zone in layout_data["zones"]:
                pct = zone["fraction"] * 100
                st.markdown(f"**{zone['name']}** — {pct:.1f}% ({zone['sqm']:,.0f} m² / {zone['sqft']:,.0f} sqft)")
        ...
        st.download_button(label="⬇️ Download Premium PDF Farm Plan", data=pdf_bytes, file_name=filename, mime="application/pdf", use_container_width=True)
    else:
        st.markdown("""
            <div style="text-align:center; padding:3rem 1rem; color:#717D7E;">
                ...
                <div style="margin-top:2rem; display:flex; justify-content:center; gap:2rem; flex-wrap:wrap;">
                    <div style="background:white;border-radius:10px;padding:1rem 1.5rem;box-shadow:0 2px 8px rgba(0,0,0,0.08);min-width:140px;">
